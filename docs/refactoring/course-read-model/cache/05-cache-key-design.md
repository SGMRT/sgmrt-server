...
> ⚠️ **대체됨 (2026-08-06)**: 이 설계는 [`07-cell-bucket-design.md`](07-cell-bucket-design.md)(셀 버킷)로 대체 결정됐다.
> 실사용 트래픽 리플레이에서 regionId의 히트율 우위가 재현되지 않았고(공유 히트 63 vs 지오해싱 72),
> 셀 버킷이 DB 회피율·이빅트 단순성·정확도 모두에서 앞섰다 (07 §2). 이 문서는 결정 이력으로 보존한다.
...
| `regionId` 있음 + `radiusM` 범위 밖 (<1,000 — 음수 포함, 또는 >3,000) | 비캐시 폴백 (**반경 호환 방어** — 캐시 값은 고정 2km라 광역 뷰포트엔 부족하고 좁은 반경엔 과잉인 침묵 오답, §4) |
