...
7. **테스트는 핵심 로직만** — Red 단계는 도메인 불변식·비즈니스 규칙·경계 조건(깨지면 실제로 아픈 것)에 집중한다. 단순 위임/게터/보일러플레이트/프레임워크 자체 동작(어노테이션 존재, 매핑 나열식 검증 등) 테스트는 만들지 않는다. 시나리오를 망라하기보다 핵심 경로당 대표 케이스 + 위험한 경계만. Red 프롬프트에 이 원칙을 명시해서 전달한다.
8. **5분 간격 진행 ping** — 루프 시작(Phase 0 완료) 직후 `ScheduleWakeup`(delaySeconds: 300)으로 진행 보고를 예약한다. 각 ping에서 현재 Phase/Step·완료·진행 중을 간결히 보고한 뒤 300초 뒤 ping을 재예약하고, Phase 5 최종 보고가 끝났거나 사용자가 중단을 요청하면 `stop: true`로 예약을 중단한다. ping은 진행 중인 에이전트/빌드를 방해하지 않는다.
