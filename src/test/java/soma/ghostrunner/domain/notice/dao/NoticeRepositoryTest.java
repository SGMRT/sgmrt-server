...
class NoticeRepositoryTest extends IntegrationTestSupport {
    ...
    @BeforeEach
    void setUp() {
        testMember = memberRepository.save(Member.of("테스트 유저", "test-url"));
    }
    ...
    class ActivationByTimeTest {
        ...
        void should_find_active_notice() {
            ...
            // when
            List<Notice> result = noticeRepository.findActiveNoticesForMember(baseDateTime, testMember.getUuid(), null);
    ...
    @Nested
    @DisplayName("사용자 닫음 여부 검증")
    class DismissalStatusTest {

        @Test
        @DisplayName("사용자가 닫지 않은 활성 공지는 조회되어야 한다.")
        ...
        @Test
        @DisplayName("사용자가 '영구적으로' 닫은 활성 공지는 조회되지 않아야 한다.")
        void should_not_find_notice_when_permanently_dismissed() {
            ...
            noticeRepository.save(notice);
            noticeDismissalRepository.save(createDismissal(testMember, notice, null)); // dismissUntil = null (영구)
        ...
        @Test
        @DisplayName("사용자가 닫았지만 '닫음 기간이 만료된' 활성 공지는 다시 조회되어야 한다.")
        void should_find_notice_when_dismissal_is_expired() {
            ...
            noticeRepository.save(notice);
            // '닫음' 기간이 어제로 만료됨
            noticeDismissalRepository.save(createDismissal(testMember, notice, baseDateTime.minusDays(1)));
    ...
    class NoticeTypeFilterTest {
        ...
        void should_find_only_specific_type_notices() {
            ...
            // when
            List<Notice> result = noticeRepository.findActiveNoticesForMember(baseDateTime, testMember.getUuid(), NoticeType.EVENT);
