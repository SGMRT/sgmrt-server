...
class PacemakerServiceTest {
    ...
    @DisplayName("성공 흐름: 검증 → VDOT 조회 → 페이스 계산 → 워크아웃 생성 → 저장 → LLM 호출")
    @Test
    void 성공_흐름_오케스트레이션() throws Exception {
        ...
        member.setUuid(uuid);
        Long courseId = 1L;
        PacemakerCreateCommand cmd =
                new PacemakerCreateCommand(PacemakerType.MARATHON, 10.0, 5, 30, courseId);

        when(memberService.findMemberByUuid(uuid)).thenReturn(member);
        
        // Validator는 예외를 던지지 않으면 통과 (코스 검증만)
        doNothing().when(validator).validateCreationRequest(courseId);
        
        when(memberService.findMemberVdot(uuid)).thenReturn(30);

        when(vdotService.getExpectedPacesByVdot(30))
        ...
        InOrder inOrder = inOrder(memberService, validator, vdotService, workoutService,
                pacemakerRepository, llmService);
        inOrder.verify(memberService).findMemberByUuid(uuid);
        inOrder.verify(validator).validateCreationRequest(courseId);
        inOrder.verify(memberService).findMemberVdot(uuid);
        inOrder.verify(vdotService).getExpectedPacesByVdot(30);
    ...
    void getPacemaker_completed_success() {
        ...
        verifyNoInteractions(llmService, vdotService, workoutService, memberService, redisRunningRepository, validator);
    }
    ...
    void getRateLimitCounter() {
        ...
        // then
        assertThat(remainedRateLimitCount).isEqualTo(PacemakerService.DAILY_LIMIT - 1);
    }
    ...
    void getDailyMaxLimitCountWhenNotCreatePacemaker() {
        ...
        // then
        assertThat(remainedRateLimitCount).isEqualTo(PacemakerService.DAILY_LIMIT);
    }
